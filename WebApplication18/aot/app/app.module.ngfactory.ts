/**
 * @fileoverview This file is generated by the Angular 2 template compiler.
 * Do not edit.
 * @suppress {suspiciousCode,uselessCode,missingProperties}
 */
 /* tslint:disable */

import * as import0 from '@angular/core/src/linker/ng_module_factory';
import * as import1 from '../../app/app.module';
import * as import2 from '@angular/common/src/common_module';
import * as import3 from '@angular/core/src/application_module';
import * as import4 from '@angular/platform-browser/src/browser';
import * as import5 from '@angular/http/src/http_module';
import * as import6 from '@angular/router/src/router_module';
import * as import7 from '../../app/app.routes';
import * as import8 from '@angular/forms/src/directives';
import * as import9 from '@angular/forms/src/form_providers';
import * as import10 from '../../app/shared.module';
import * as import11 from '@angular/common/src/localization';
import * as import12 from '@angular/core/src/application_init';
import * as import13 from '@angular/core/src/testability/testability';
import * as import14 from '@angular/core/src/application_ref';
import * as import15 from '@angular/core/src/linker/compiler';
import * as import16 from '@angular/platform-browser/src/dom/events/hammer_gestures';
import * as import17 from '@angular/platform-browser/src/dom/events/event_manager';
import * as import18 from '@angular/platform-browser/src/dom/shared_styles_host';
import * as import19 from '@angular/platform-browser/src/dom/dom_renderer';
import * as import20 from '@angular/platform-browser/src/security/dom_sanitization_service';
import * as import21 from '@angular/core/src/linker/view_utils';
import * as import22 from '@angular/platform-browser/src/browser/title';
import * as import23 from '@angular/http/src/backends/browser_xhr';
import * as import24 from '@angular/http/src/base_response_options';
import * as import25 from '@angular/http/src/backends/xhr_backend';
import * as import26 from '@angular/http/src/base_request_options';
import * as import27 from '@angular/common/src/location/location';
import * as import28 from '@angular/router/src/url_tree';
import * as import29 from '@angular/router/src/router_outlet_map';
import * as import30 from '@angular/core/src/linker/system_js_ng_module_factory_loader';
import * as import31 from '@angular/router/src/router_preloader';
import * as import32 from '../../app/common/can-deactivate-guard.service';
import * as import33 from '@angular/forms/src/directives/radio_control_value_accessor';
import * as import34 from '../../app/services/page-name.service';
import * as import35 from '../../app/services/services';
import * as import36 from '@angular/core/src/di/injector';
import * as import37 from './Home/home.ngfactory';
import * as import38 from './Biography/biography.ngfactory';
import * as import39 from './Pictures/pictures.ngfactory';
import * as import40 from './Videos/videos.ngfactory';
import * as import41 from './Programs/programs.ngfactory';
import * as import42 from './Links/links.ngfactory';
import * as import43 from './Contact/contact.ngfactory';
import * as import44 from './app.component.ngfactory';
import * as import45 from '@angular/core/src/application_tokens';
import * as import46 from '@angular/platform-browser/src/dom/events/dom_events';
import * as import47 from '@angular/platform-browser/src/dom/events/key_events';
import * as import48 from '@angular/core/src/zone/ng_zone';
import * as import49 from '@angular/platform-browser/src/dom/debug/ng_probe';
import * as import50 from '@angular/common/src/location/platform_location';
import * as import51 from '@angular/common/src/location/location_strategy';
import * as import52 from '../../app/Home/home';
import * as import53 from '../../app/Biography/biography';
import * as import54 from '../../app/Pictures/pictures';
import * as import55 from '../../app/Videos/videos';
import * as import56 from '../../app/Programs/programs';
import * as import57 from '../../app/Links/links';
import * as import58 from '../../app/Contact/contact';
import * as import59 from '@angular/router/src/url_handling_strategy';
import * as import60 from '@angular/router/src/router';
import * as import61 from '@angular/core/src/console';
import * as import62 from '@angular/core/src/i18n/tokens';
import * as import63 from '@angular/core/src/error_handler';
import * as import64 from '@angular/platform-browser/src/dom/dom_tokens';
import * as import65 from '@angular/platform-browser/src/dom/animation_driver';
import * as import66 from '@angular/core/src/render/api';
import * as import67 from '@angular/core/src/security';
import * as import68 from '@angular/core/src/change_detection/differs/iterable_differs';
import * as import69 from '@angular/core/src/change_detection/differs/keyvalue_differs';
import * as import70 from '@angular/http/src/interfaces';
import * as import71 from '@angular/http/src/http';
import * as import72 from '@angular/core/src/linker/ng_module_factory_loader';
import * as import73 from '@angular/router/src/router_config_loader';
import * as import74 from '@angular/router/src/router_state';
class AppModuleInjector extends import0.NgModuleInjector<import1.AppModule> {
  _CommonModule_0:import2.CommonModule;
  _ApplicationModule_1:import3.ApplicationModule;
  _BrowserModule_2:import4.BrowserModule;
  _HttpModule_3:import5.HttpModule;
  _ROUTER_FORROOT_GUARD_4:any;
  _RouterModule_5:import6.RouterModule;
  _AppRoutingModule_6:import7.AppRoutingModule;
  _InternalFormsSharedModule_7:import8.InternalFormsSharedModule;
  _FormsModule_8:import9.FormsModule;
  _SharedModule_9:import10.SharedModule;
  _AppModule_10:import1.AppModule;
  __LOCALE_ID_11:any;
  __NgLocalization_12:import11.NgLocaleLocalization;
  _ErrorHandler_13:any;
  _ApplicationInitStatus_14:import12.ApplicationInitStatus;
  _Testability_15:import13.Testability;
  _ApplicationRef__16:import14.ApplicationRef_;
  __ApplicationRef_17:any;
  __Compiler_18:import15.Compiler;
  __APP_ID_19:any;
  __DOCUMENT_20:any;
  __HAMMER_GESTURE_CONFIG_21:import16.HammerGestureConfig;
  __EVENT_MANAGER_PLUGINS_22:any[];
  __EventManager_23:import17.EventManager;
  __DomSharedStylesHost_24:import18.DomSharedStylesHost;
  __AnimationDriver_25:any;
  __DomRootRenderer_26:import19.DomRootRenderer_;
  __RootRenderer_27:any;
  __DomSanitizer_28:import20.DomSanitizerImpl;
  __Sanitizer_29:any;
  __ViewUtils_30:import21.ViewUtils;
  __IterableDiffers_31:any;
  __KeyValueDiffers_32:any;
  __SharedStylesHost_33:any;
  __Title_34:import22.Title;
  __BrowserXhr_35:import23.BrowserXhr;
  __ResponseOptions_36:import24.BaseResponseOptions;
  __XSRFStrategy_37:any;
  __XHRBackend_38:import25.XHRBackend;
  __RequestOptions_39:import26.BaseRequestOptions;
  __Http_40:any;
  __ROUTER_CONFIGURATION_41:any;
  __LocationStrategy_42:any;
  __Location_43:import27.Location;
  __UrlSerializer_44:import28.DefaultUrlSerializer;
  __RouterOutletMap_45:import29.RouterOutletMap;
  __NgModuleFactoryLoader_46:import30.SystemJsNgModuleLoader;
  __ROUTES_47:any[];
  __Router_48:any;
  __ActivatedRoute_49:any;
  _NoPreloading_50:import31.NoPreloading;
  _PreloadingStrategy_51:any;
  _RouterPreloader_52:import31.RouterPreloader;
  __PreloadAllModules_53:import31.PreloadAllModules;
  __ROUTER_INITIALIZER_54:any;
  __APP_BOOTSTRAP_LISTENER_55:any[];
  __CanDeactivateGuard_56:import32.CanDeactivateGuard;
  __RadioControlRegistry_57:import33.RadioControlRegistry;
  __pageNameService_58:import34.pageNameService;
  __CacheManager_59:import35.CacheManager;
  __DataService_60:import35.DataService;
  __DialogService_61:import35.DialogService;
  __LogService_62:import35.LogService;
  __TranslationService_63:import35.TranslationService;
  constructor(parent:import36.Injector) {
    super(parent,[
      import37.HomeNgFactory,
      import38.BiographyNgFactory,
      import39.PicturesNgFactory,
      import40.VideosNgFactory,
      import41.ProgramsNgFactory,
      import42.LinksNgFactory,
      import43.ContactNgFactory,
      import44.AppComponentNgFactory
    ]
    ,[import44.AppComponentNgFactory]);
  }
  get _LOCALE_ID_11():any {
    if ((this.__LOCALE_ID_11 == null)) { (this.__LOCALE_ID_11 = 'en-US'); }
    return this.__LOCALE_ID_11;
  }
  get _NgLocalization_12():import11.NgLocaleLocalization {
    if ((this.__NgLocalization_12 == null)) { (this.__NgLocalization_12 = new import11.NgLocaleLocalization(this._LOCALE_ID_11)); }
    return this.__NgLocalization_12;
  }
  get _ApplicationRef_17():any {
    if ((this.__ApplicationRef_17 == null)) { (this.__ApplicationRef_17 = this._ApplicationRef__16); }
    return this.__ApplicationRef_17;
  }
  get _Compiler_18():import15.Compiler {
    if ((this.__Compiler_18 == null)) { (this.__Compiler_18 = new import15.Compiler()); }
    return this.__Compiler_18;
  }
  get _APP_ID_19():any {
    if ((this.__APP_ID_19 == null)) { (this.__APP_ID_19 = import45._appIdRandomProviderFactory()); }
    return this.__APP_ID_19;
  }
  get _DOCUMENT_20():any {
    if ((this.__DOCUMENT_20 == null)) { (this.__DOCUMENT_20 = import4._document()); }
    return this.__DOCUMENT_20;
  }
  get _HAMMER_GESTURE_CONFIG_21():import16.HammerGestureConfig {
    if ((this.__HAMMER_GESTURE_CONFIG_21 == null)) { (this.__HAMMER_GESTURE_CONFIG_21 = new import16.HammerGestureConfig()); }
    return this.__HAMMER_GESTURE_CONFIG_21;
  }
  get _EVENT_MANAGER_PLUGINS_22():any[] {
    if ((this.__EVENT_MANAGER_PLUGINS_22 == null)) { (this.__EVENT_MANAGER_PLUGINS_22 = [
      new import46.DomEventsPlugin(),
      new import47.KeyEventsPlugin(),
      new import16.HammerGesturesPlugin(this._HAMMER_GESTURE_CONFIG_21)
    ]
    ); }
    return this.__EVENT_MANAGER_PLUGINS_22;
  }
  get _EventManager_23():import17.EventManager {
    if ((this.__EventManager_23 == null)) { (this.__EventManager_23 = new import17.EventManager(this._EVENT_MANAGER_PLUGINS_22,this.parent.get(import48.NgZone))); }
    return this.__EventManager_23;
  }
  get _DomSharedStylesHost_24():import18.DomSharedStylesHost {
    if ((this.__DomSharedStylesHost_24 == null)) { (this.__DomSharedStylesHost_24 = new import18.DomSharedStylesHost(this._DOCUMENT_20)); }
    return this.__DomSharedStylesHost_24;
  }
  get _AnimationDriver_25():any {
    if ((this.__AnimationDriver_25 == null)) { (this.__AnimationDriver_25 = import4._resolveDefaultAnimationDriver()); }
    return this.__AnimationDriver_25;
  }
  get _DomRootRenderer_26():import19.DomRootRenderer_ {
    if ((this.__DomRootRenderer_26 == null)) { (this.__DomRootRenderer_26 = new import19.DomRootRenderer_(this._DOCUMENT_20,this._EventManager_23,this._DomSharedStylesHost_24,this._AnimationDriver_25,this._APP_ID_19)); }
    return this.__DomRootRenderer_26;
  }
  get _RootRenderer_27():any {
    if ((this.__RootRenderer_27 == null)) { (this.__RootRenderer_27 = import49._createConditionalRootRenderer(this._DomRootRenderer_26,this.parent.get(import49.NgProbeToken,(null as any)))); }
    return this.__RootRenderer_27;
  }
  get _DomSanitizer_28():import20.DomSanitizerImpl {
    if ((this.__DomSanitizer_28 == null)) { (this.__DomSanitizer_28 = new import20.DomSanitizerImpl()); }
    return this.__DomSanitizer_28;
  }
  get _Sanitizer_29():any {
    if ((this.__Sanitizer_29 == null)) { (this.__Sanitizer_29 = this._DomSanitizer_28); }
    return this.__Sanitizer_29;
  }
  get _ViewUtils_30():import21.ViewUtils {
    if ((this.__ViewUtils_30 == null)) { (this.__ViewUtils_30 = new import21.ViewUtils(this._RootRenderer_27,this._Sanitizer_29)); }
    return this.__ViewUtils_30;
  }
  get _IterableDiffers_31():any {
    if ((this.__IterableDiffers_31 == null)) { (this.__IterableDiffers_31 = import3._iterableDiffersFactory()); }
    return this.__IterableDiffers_31;
  }
  get _KeyValueDiffers_32():any {
    if ((this.__KeyValueDiffers_32 == null)) { (this.__KeyValueDiffers_32 = import3._keyValueDiffersFactory()); }
    return this.__KeyValueDiffers_32;
  }
  get _SharedStylesHost_33():any {
    if ((this.__SharedStylesHost_33 == null)) { (this.__SharedStylesHost_33 = this._DomSharedStylesHost_24); }
    return this.__SharedStylesHost_33;
  }
  get _Title_34():import22.Title {
    if ((this.__Title_34 == null)) { (this.__Title_34 = new import22.Title()); }
    return this.__Title_34;
  }
  get _BrowserXhr_35():import23.BrowserXhr {
    if ((this.__BrowserXhr_35 == null)) { (this.__BrowserXhr_35 = new import23.BrowserXhr()); }
    return this.__BrowserXhr_35;
  }
  get _ResponseOptions_36():import24.BaseResponseOptions {
    if ((this.__ResponseOptions_36 == null)) { (this.__ResponseOptions_36 = new import24.BaseResponseOptions()); }
    return this.__ResponseOptions_36;
  }
  get _XSRFStrategy_37():any {
    if ((this.__XSRFStrategy_37 == null)) { (this.__XSRFStrategy_37 = import5._createDefaultCookieXSRFStrategy()); }
    return this.__XSRFStrategy_37;
  }
  get _XHRBackend_38():import25.XHRBackend {
    if ((this.__XHRBackend_38 == null)) { (this.__XHRBackend_38 = new import25.XHRBackend(this._BrowserXhr_35,this._ResponseOptions_36,this._XSRFStrategy_37)); }
    return this.__XHRBackend_38;
  }
  get _RequestOptions_39():import26.BaseRequestOptions {
    if ((this.__RequestOptions_39 == null)) { (this.__RequestOptions_39 = new import26.BaseRequestOptions()); }
    return this.__RequestOptions_39;
  }
  get _Http_40():any {
    if ((this.__Http_40 == null)) { (this.__Http_40 = import5.httpFactory(this._XHRBackend_38,this._RequestOptions_39)); }
    return this.__Http_40;
  }
  get _ROUTER_CONFIGURATION_41():any {
    if ((this.__ROUTER_CONFIGURATION_41 == null)) { (this.__ROUTER_CONFIGURATION_41 = {useHash: true}); }
    return this.__ROUTER_CONFIGURATION_41;
  }
  get _LocationStrategy_42():any {
    if ((this.__LocationStrategy_42 == null)) { (this.__LocationStrategy_42 = import6.provideLocationStrategy(this.parent.get(import50.PlatformLocation),this.parent.get(import51.APP_BASE_HREF,(null as any)),this._ROUTER_CONFIGURATION_41)); }
    return this.__LocationStrategy_42;
  }
  get _Location_43():import27.Location {
    if ((this.__Location_43 == null)) { (this.__Location_43 = new import27.Location(this._LocationStrategy_42)); }
    return this.__Location_43;
  }
  get _UrlSerializer_44():import28.DefaultUrlSerializer {
    if ((this.__UrlSerializer_44 == null)) { (this.__UrlSerializer_44 = new import28.DefaultUrlSerializer()); }
    return this.__UrlSerializer_44;
  }
  get _RouterOutletMap_45():import29.RouterOutletMap {
    if ((this.__RouterOutletMap_45 == null)) { (this.__RouterOutletMap_45 = new import29.RouterOutletMap()); }
    return this.__RouterOutletMap_45;
  }
  get _NgModuleFactoryLoader_46():import30.SystemJsNgModuleLoader {
    if ((this.__NgModuleFactoryLoader_46 == null)) { (this.__NgModuleFactoryLoader_46 = new import30.SystemJsNgModuleLoader(this._Compiler_18,this.parent.get(import30.SystemJsNgModuleLoaderConfig,(null as any)))); }
    return this.__NgModuleFactoryLoader_46;
  }
  get _ROUTES_47():any[] {
      if ((this.__ROUTES_47 == null)) { (this.__ROUTES_47 = [[
        {
          path: 'galilu',
          loadChildren: 'app/galilu/galilu.module#GaliluModule'
        }
        ,
        {
          path: 'home',
          component: import52.Home
        }
        ,
        {
          path: 'biography',
          component: import53.Biography
        }
        ,
        {
          path: 'pictures',
          component: import54.Pictures
        }
        ,
        {
          path: 'videos',
          component: import55.Videos
        }
        ,
        {
          path: 'programs',
          component: import56.Programs
        }
        ,
        {
          path: 'links',
          component: import57.Links
        }
        ,
        {
          path: 'contact',
          component: import58.Contact,
          canDeactivate: [import32.CanDeactivateGuard]
        }
        ,
        {
          path: '',
          redirectTo: 'home',
          pathMatch: 'full'
        }

      ]
    ]); }
    return this.__ROUTES_47;
  }
  get _Router_48():any {
    if ((this.__Router_48 == null)) { (this.__Router_48 = import6.setupRouter(this._ApplicationRef_17,this._UrlSerializer_44,this._RouterOutletMap_45,this._Location_43,this,this._NgModuleFactoryLoader_46,this._Compiler_18,this._ROUTES_47,this._ROUTER_CONFIGURATION_41,this.parent.get(import59.UrlHandlingStrategy,(null as any)))); }
    return this.__Router_48;
  }
  get _ActivatedRoute_49():any {
    if ((this.__ActivatedRoute_49 == null)) { (this.__ActivatedRoute_49 = import6.rootRoute(this._Router_48)); }
    return this.__ActivatedRoute_49;
  }
  get _PreloadAllModules_53():import31.PreloadAllModules {
    if ((this.__PreloadAllModules_53 == null)) { (this.__PreloadAllModules_53 = new import31.PreloadAllModules()); }
    return this.__PreloadAllModules_53;
  }
  get _ROUTER_INITIALIZER_54():any {
    if ((this.__ROUTER_INITIALIZER_54 == null)) { (this.__ROUTER_INITIALIZER_54 = import6.initialRouterNavigation(this._Router_48,this._ApplicationRef_17,this._RouterPreloader_52,this._ROUTER_CONFIGURATION_41)); }
    return this.__ROUTER_INITIALIZER_54;
  }
  get _APP_BOOTSTRAP_LISTENER_55():any[] {
    if ((this.__APP_BOOTSTRAP_LISTENER_55 == null)) { (this.__APP_BOOTSTRAP_LISTENER_55 = [this._ROUTER_INITIALIZER_54]); }
    return this.__APP_BOOTSTRAP_LISTENER_55;
  }
  get _CanDeactivateGuard_56():import32.CanDeactivateGuard {
    if ((this.__CanDeactivateGuard_56 == null)) { (this.__CanDeactivateGuard_56 = new import32.CanDeactivateGuard()); }
    return this.__CanDeactivateGuard_56;
  }
  get _RadioControlRegistry_57():import33.RadioControlRegistry {
    if ((this.__RadioControlRegistry_57 == null)) { (this.__RadioControlRegistry_57 = new import33.RadioControlRegistry()); }
    return this.__RadioControlRegistry_57;
  }
  get _pageNameService_58():import34.pageNameService {
    if ((this.__pageNameService_58 == null)) { (this.__pageNameService_58 = new import34.pageNameService(this._Router_48)); }
    return this.__pageNameService_58;
  }
  get _CacheManager_59():import35.CacheManager {
    if ((this.__CacheManager_59 == null)) { (this.__CacheManager_59 = new import35.CacheManager()); }
    return this.__CacheManager_59;
  }
  get _DataService_60():import35.DataService {
    if ((this.__DataService_60 == null)) { (this.__DataService_60 = new import35.DataService(this._Http_40,this._CacheManager_59)); }
    return this.__DataService_60;
  }
  get _DialogService_61():import35.DialogService {
    if ((this.__DialogService_61 == null)) { (this.__DialogService_61 = new import35.DialogService()); }
    return this.__DialogService_61;
  }
  get _LogService_62():import35.LogService {
    if ((this.__LogService_62 == null)) { (this.__LogService_62 = new import35.LogService()); }
    return this.__LogService_62;
  }
  get _TranslationService_63():import35.TranslationService {
    if ((this.__TranslationService_63 == null)) { (this.__TranslationService_63 = new import35.TranslationService(this._CacheManager_59)); }
    return this.__TranslationService_63;
  }
  createInternal():import1.AppModule {
    this._CommonModule_0 = new import2.CommonModule();
    this._ApplicationModule_1 = new import3.ApplicationModule();
    this._BrowserModule_2 = new import4.BrowserModule(this.parent.get(import4.BrowserModule,(null as any)));
    this._HttpModule_3 = new import5.HttpModule();
    this._ROUTER_FORROOT_GUARD_4 = import6.provideForRootGuard(this.parent.get(import60.Router,(null as any)));
    this._RouterModule_5 = new import6.RouterModule(this._ROUTER_FORROOT_GUARD_4);
    this._AppRoutingModule_6 = new import7.AppRoutingModule();
    this._InternalFormsSharedModule_7 = new import8.InternalFormsSharedModule();
    this._FormsModule_8 = new import9.FormsModule();
    this._SharedModule_9 = new import10.SharedModule();
    this._AppModule_10 = new import1.AppModule();
    this._ErrorHandler_13 = import4.errorHandler();
    this._ApplicationInitStatus_14 = new import12.ApplicationInitStatus(this.parent.get(import12.APP_INITIALIZER,(null as any)));
    this._Testability_15 = new import13.Testability(this.parent.get(import48.NgZone));
    this._ApplicationRef__16 = new import14.ApplicationRef_(this.parent.get(import48.NgZone),this.parent.get(import61.Console),this,this._ErrorHandler_13,this,this._ApplicationInitStatus_14,this.parent.get(import13.TestabilityRegistry,(null as any)),this._Testability_15);
    this._NoPreloading_50 = new import31.NoPreloading();
    this._PreloadingStrategy_51 = this._NoPreloading_50;
    this._RouterPreloader_52 = new import31.RouterPreloader(this._Router_48,this._NgModuleFactoryLoader_46,this._Compiler_18,this,this._PreloadingStrategy_51);
    return this._AppModule_10;
  }
  getInternal(token:any,notFoundResult:any):any {
    if ((token === import2.CommonModule)) { return this._CommonModule_0; }
    if ((token === import3.ApplicationModule)) { return this._ApplicationModule_1; }
    if ((token === import4.BrowserModule)) { return this._BrowserModule_2; }
    if ((token === import5.HttpModule)) { return this._HttpModule_3; }
    if ((token === import6.ROUTER_FORROOT_GUARD)) { return this._ROUTER_FORROOT_GUARD_4; }
    if ((token === import6.RouterModule)) { return this._RouterModule_5; }
    if ((token === import7.AppRoutingModule)) { return this._AppRoutingModule_6; }
    if ((token === import8.InternalFormsSharedModule)) { return this._InternalFormsSharedModule_7; }
    if ((token === import9.FormsModule)) { return this._FormsModule_8; }
    if ((token === import10.SharedModule)) { return this._SharedModule_9; }
    if ((token === import1.AppModule)) { return this._AppModule_10; }
    if ((token === import62.LOCALE_ID)) { return this._LOCALE_ID_11; }
    if ((token === import11.NgLocalization)) { return this._NgLocalization_12; }
    if ((token === import63.ErrorHandler)) { return this._ErrorHandler_13; }
    if ((token === import12.ApplicationInitStatus)) { return this._ApplicationInitStatus_14; }
    if ((token === import13.Testability)) { return this._Testability_15; }
    if ((token === import14.ApplicationRef_)) { return this._ApplicationRef__16; }
    if ((token === import14.ApplicationRef)) { return this._ApplicationRef_17; }
    if ((token === import15.Compiler)) { return this._Compiler_18; }
    if ((token === import45.APP_ID)) { return this._APP_ID_19; }
    if ((token === import64.DOCUMENT)) { return this._DOCUMENT_20; }
    if ((token === import16.HAMMER_GESTURE_CONFIG)) { return this._HAMMER_GESTURE_CONFIG_21; }
    if ((token === import17.EVENT_MANAGER_PLUGINS)) { return this._EVENT_MANAGER_PLUGINS_22; }
    if ((token === import17.EventManager)) { return this._EventManager_23; }
    if ((token === import18.DomSharedStylesHost)) { return this._DomSharedStylesHost_24; }
    if ((token === import65.AnimationDriver)) { return this._AnimationDriver_25; }
    if ((token === import19.DomRootRenderer)) { return this._DomRootRenderer_26; }
    if ((token === import66.RootRenderer)) { return this._RootRenderer_27; }
    if ((token === import20.DomSanitizer)) { return this._DomSanitizer_28; }
    if ((token === import67.Sanitizer)) { return this._Sanitizer_29; }
    if ((token === import21.ViewUtils)) { return this._ViewUtils_30; }
    if ((token === import68.IterableDiffers)) { return this._IterableDiffers_31; }
    if ((token === import69.KeyValueDiffers)) { return this._KeyValueDiffers_32; }
    if ((token === import18.SharedStylesHost)) { return this._SharedStylesHost_33; }
    if ((token === import22.Title)) { return this._Title_34; }
    if ((token === import23.BrowserXhr)) { return this._BrowserXhr_35; }
    if ((token === import24.ResponseOptions)) { return this._ResponseOptions_36; }
    if ((token === import70.XSRFStrategy)) { return this._XSRFStrategy_37; }
    if ((token === import25.XHRBackend)) { return this._XHRBackend_38; }
    if ((token === import26.RequestOptions)) { return this._RequestOptions_39; }
    if ((token === import71.Http)) { return this._Http_40; }
    if ((token === import6.ROUTER_CONFIGURATION)) { return this._ROUTER_CONFIGURATION_41; }
    if ((token === import51.LocationStrategy)) { return this._LocationStrategy_42; }
    if ((token === import27.Location)) { return this._Location_43; }
    if ((token === import28.UrlSerializer)) { return this._UrlSerializer_44; }
    if ((token === import29.RouterOutletMap)) { return this._RouterOutletMap_45; }
    if ((token === import72.NgModuleFactoryLoader)) { return this._NgModuleFactoryLoader_46; }
    if ((token === import73.ROUTES)) { return this._ROUTES_47; }
    if ((token === import60.Router)) { return this._Router_48; }
    if ((token === import74.ActivatedRoute)) { return this._ActivatedRoute_49; }
    if ((token === import31.NoPreloading)) { return this._NoPreloading_50; }
    if ((token === import31.PreloadingStrategy)) { return this._PreloadingStrategy_51; }
    if ((token === import31.RouterPreloader)) { return this._RouterPreloader_52; }
    if ((token === import31.PreloadAllModules)) { return this._PreloadAllModules_53; }
    if ((token === import6.ROUTER_INITIALIZER)) { return this._ROUTER_INITIALIZER_54; }
    if ((token === import45.APP_BOOTSTRAP_LISTENER)) { return this._APP_BOOTSTRAP_LISTENER_55; }
    if ((token === import32.CanDeactivateGuard)) { return this._CanDeactivateGuard_56; }
    if ((token === import33.RadioControlRegistry)) { return this._RadioControlRegistry_57; }
    if ((token === import34.pageNameService)) { return this._pageNameService_58; }
    if ((token === import35.CacheManager)) { return this._CacheManager_59; }
    if ((token === import35.DataService)) { return this._DataService_60; }
    if ((token === import35.DialogService)) { return this._DialogService_61; }
    if ((token === import35.LogService)) { return this._LogService_62; }
    if ((token === import35.TranslationService)) { return this._TranslationService_63; }
    return notFoundResult;
  }
  destroyInternal():void {
    this._ApplicationRef__16.ngOnDestroy();
    this._RouterPreloader_52.ngOnDestroy();
  }
}
export const AppModuleNgFactory:import0.NgModuleFactory<import1.AppModule> = new import0.NgModuleFactory(AppModuleInjector,import1.AppModule);